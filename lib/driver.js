...
Driver.prototype.do = function (queryList, opts, cb) {
	...
	function queue () {
		...
		self.doSingleQuery (query, undefined, opts, function (err, rows) {
			...
			if (((!err) || err && ignoreErrors) && queryQueue.length) {
				queue ();
				return;
			}
...
/**
 * run single query
 * @param {object}       query    query string or object with {text: SQL, type: 'SELECT', scope: 'DDL|DML'}
...
Driver.prototype.doSingleQuery = function (query, bindings, options, cb) {
	...
	var q = query.text ? query.text : query;
	var qType = query.type ? query.type : q.match (/[\r\n\t\s]*SELECT/im) ? 'SELECT' : null;

	if (qType === 'SELECT') {

		this.fetchAll (q, bindings, options, cb);

	} else {

		this.execute (q, bindings, options, cb);
...
Driver.prototype.connect = function (opts, cb) {
	...
	if (connParams.tunnel) {
		...
		var tunnel = new Tunnel (connParams, function (err, config) {
